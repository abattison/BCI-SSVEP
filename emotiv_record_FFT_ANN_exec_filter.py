...
bot_freq = 8
top_freq = 11
record_freq = "both" # valid options are top_freq, bot_freq, "both" ("both" should not be used for training)
...
def main():
    ...
    for x in range(500):
        ...
        if from_file:
            ...
            if i > 29:
                out_prob = ann.run(input_d)
                top_total += out_prob[0]
                bot_total += out_prob[1]
                sample_count += 1
                print out_prob
    ...
    print str(top_total/sample_count) + " " + str(bot_total/sample_count)
