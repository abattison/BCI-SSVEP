...
def main():
    ...
    for x in range(500):
        ...
        probe_channel = 9   # channel number 1 - 15 from the emotiv device

        amp = abs(fft.fft(data[probe_channel-1]))   # this might have to be numpy.absolute()
        timestep = times[1]-times[0]    # the distance between the samples
        frequencies = fft.fftfreq(amp.size, d=timestep)
        channel = ['FC3', 'FCZ', 'FC4', 'CP3', 'CPZ', 'CP4', 'PO7', 'PO3', 'POZ', 'PO4', 'PO8', 'POZ', 'O1', 'O2']
    ...
    with open('15Hz_both.dat', 'wb') as handle:
